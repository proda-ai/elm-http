effect module Compat.Http where { command = MyCmd, subscription = MySub } exposing
    ( get, post, request
    , Header, header
...
jsonBody value =
    Native.Compat.Http.pair "application/json" (Encode.encode 0 value)
...
multipartBody parts =
    Native.Compat.Http.pair "" (Native.Compat.Http.toFormData parts)
...
bytesPart key mime bytes =
    Native.Compat.Http.pair key (Native.Compat.Http.bytesToBlob mime bytes)
...
expectJson toMsg decoder =
    expectStringResponse toMsg <|
        resolve <|
            \string ->
                Result.mapError identity (Decode.decodeString decoder string)
...
expectStringResponse toMsg toResult =
    Native.Compat.Http.expect "" identity (toResult >> toMsg)
...
expectBytesResponse toMsg toResult =
    Native.Compat.Http.expect "arraybuffer" Native.Compat.Http.toDataView (toResult >> toMsg)
...
stringResolver =
    Native.Compat.Http.expect "" identity
...
bytesResolver =
    Native.Compat.Http.expect "arraybuffer" Native.Compat.Http.toDataView
    ...
    case cmd of
        ...
        Request r ->
            Request
                ...
                , url = r.url
                , body = r.body
                , expect = Native.Compat.Http.mapExpect func r.expect
                , timeout = r.timeout
                , tracker = r.tracker
    ...
    case cmds of
        ...
        cmd :: otherCmds ->
            case cmd of
                ...
                Request req ->
                    Process.spawn (Native.Compat.Http.toTask router (Platform.sendToApp router) req)
                        |> Task.andThen
                            (\pid ->
                                case req.tracker of
                                    Nothing ->
